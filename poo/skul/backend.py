...
class Sala:
    def __init__(self, id, num,filas, colum,tipo):
        self.idsala=id
        self.num=num
        self.asientos=[[False for _ in range(colum)]for _ in range (filas)]
        self.tipo=tipo
        
    def mostrar_dispo(self):
        return self.asientos
    
    def reservar_asientos(self,fila,colum):
        if not self.asientos[fila][colum]:
            self.asientos[fila][colum]=True
            return True
        return False
